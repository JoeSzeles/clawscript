...
function getPort() {
  if (activeBrainName !== 'default') {
    if (runningProcesses[activeBrainName]) {
      return runningProcesses[activeBrainName].port;
    }
    throw new Error('Active brain "' + activeBrainName + '" is no longer running. Use BRAIN_USE "default" to reset.');
  }
    ...
    const req = http.request(opts, (res) => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode >= 400) {
          ...
          reject(new Error('Brain request failed (' + res.statusCode + '): ' + msg));
        } else {
          resolve(parsed);
        }
      });
